...
local evaluate_hand_part1 = function(hand)
	local card_counts = {}
	local keys = {}
...
---@param sets table
---@param evaluate_fn function
---@return table buckets
local put_into_buckets = function(sets, evaluate_fn)
	local buckets = {}
	for _, set in ipairs(sets) do
		local hand = evaluate_fn(set.hand)
...
local is_lower = function(hand1, hand2, cards)
	local cards1 = {}
	local cards2 = {}
...
local part_1 = function()
	...
	local buckets = put_into_buckets(sets, evaluate_hand_part1)

	---@param bucket table { { hand, bid } }
	---@return table sorted_bucket
	local sort = function(bucket)
		table.sort(bucket, function(a, b)
			return is_lower(a.hand, b.hand, cards_part1)
		end)
		return bucket
	end
...
---@param hand string
---@return number type Type
local evaluate_hand_part2 = function(hand)
	local card_counts = {}
	local keys = {}
	for c in string.gmatch(hand, ".") do
		if card_counts[c] == nil then
	...
	-- if J exists, fit it in
	if card_counts["J"] ~= nil then
		if size(keys) == 1 or size(keys) == 2 then
			return Type.FIVE
		elseif size(keys) == 3 then
			if card_counts[keys[1]] == card_counts[keys[2]] and keys[3] == "J" then
				return Type.HOUSE
			else
				return Type.FOUR
			end
		elseif size(keys) == 4 then
			return Type.THREE
		else
			return Type.ONE
		end
...
local part_2 = function()
	local sets = parse_sets()
	local buckets = put_into_buckets(sets, evaluate_hand_part2)

	---@param bucket table { { hand, bid } }
	---@return table sorted_bucket
	local sort = function(bucket)
		table.sort(bucket, function(a, b)
			return is_lower(a.hand, b.hand, cards_part2)
		end)
		return bucket
	end
